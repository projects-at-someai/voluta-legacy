...
_Last Updated:_ 6th November, 2020

Ensembles extends Apple's Core Data framework to add synchronization for macOS and iOS. Multiple SQLite persistent stores can be coupled together via file synchronization platforms like iCloud, CloudKit, Dropbox, and even direct peer-to-peer connections. The framework can be readily extended to support any service capable of moving files between devices, including custom servers.
...
Ensembles is an Objective-C framework &mdash; with Swift support &mdash; that extends Apple's Core Data framework to add peer-to-peer synchronization for macOS and iOS. Multiple SQLite persistent stores can be coupled together via a file synchronization platform like iCloud or Dropbox. The framework can be readily extended to support any service capable of moving files between devices, including custom servers.
...
Contact [support@ensembles.io](mailto:support@ensembles.io) if you have purchased an Ensembles 2 license, and need access to the GitHub repository.

Once you have access to the Ensembles 2 Git repository, you can clone Ensembles to your local drive, using the command
...
3. In the General tab, click the + button in the _Frameworks, Libraries and Embedded Content_ section.
...
By default, Ensembles only includes support for iCloud Drive. To use other cloud services, such as Dropbox or CloudKit, you may need to add a few steps to the procedure above. 
 ...
 * iCloud (Core)
 * CloudKit (CloudKit)
 * Dropbox v1 API (Dropbox) 
 * Dropbox v2 API (DropboxV2) 
 * WebDAV (WebDAV)
...
The Zip Compression backend is not actually for online storage, but can be used to wrap any of the other backends in order to compress files before transporting them.
...
There is an Ensembles book (beta) available for free [here](https://leanpub.com/ensembles).
