...
- (void)viewDidAppear:(BOOL)animated {
    ...
    //[defaults removeObjectForKey:@"HasVIP"];
...
- (void)CheckForOutOfForms {
    ...
    if (!hasForms && !hasSubscription) {
        ...
        if ([defaults objectForKey:OUT_OF_FORMS_DATE] == nil) {

            [defaults setObject:[NSDate date] forKey:OUT_OF_FORMS_DATE];
        }
...
- (void)CheckSharedDirectory {
    ...
    if(shareddir) {
        ...
        if([FileManager fileExistsAtPath:settingsfile isDirectory:&isDirectory]) {

            NSLog(@"Found MRF settings file. Prompt to load settings");

            UIAlertController *alert = [self.Alerts CreateYesNoAlert:^(UIAlertAction *action)
            {
                //Transfer settings
                NSLog(@"Importing MRF settings");
                SettingsBackupManager *backupmanager = [[SettingsBackupManager alloc] init];
                backupmanager.delegate = self;

                [backupmanager RestoreSettings:@"MRFSettings" withPath:shareddir];

                NSLog(@"Importing MRF database");
                NSString *databasefile = [NSString stringWithFormat:@"%@/MRFDatabase", shareddir];
                CoreDataManager *coredatamgr = [[SharedData SharedInstance] GetCoreDataManager];
                [coredatamgr RestoreBackupCoreData:databasefile];

                /*
                NSLog(@"Transferring forms");
                //Transfer forms
                InAppPurchaseManager *mgnr = [[SharedData SharedInstance] GetIAPManager];
                [mgnr TransferSharedIntoIAP];
                */
            ...
            withTitle:@"MRF Detected" withMessage:@"We noticed that you have MRF installed on this iPad. Would you like to import the settings? This will import all settings except for your cloud service. You will still need to link that again in the CRF settings. Your purchases and forms will also be imported"];
        ...
        //Check for CRF file
        NSString *crffile = [NSString stringWithFormat:@"%@/CRF",shareddir];

        if(![FileManager fileExistsAtPath:crffile isDirectory:&isDirectory]) {

            NSLog(@"No CRF File Found");

            NSString *str = @"CRF was here";

            [str writeToFile:[NSString stringWithFormat:@"%@/CRF",shareddir] atomically:NO encoding:NSASCIIStringEncoding error:nil];
...
- (void)CheckAndServicePendingUploads {
    ...
    if (!hasForms && !hasSubscription && (_NumPending > 0)) {
        ...
        if (_NumPending >= OUT_OF_FORMS_PENDING_LIMIT) {

            [Answers logCustomEventWithName:@"Max Pending Warning"
                           customAttributes:@{}];

            UIAlertController *alert = [self.Alerts CreateOKAlert:^(UIAlertAction *action){} withTitle:@"Error" withMessage:@"You have the max number of pending waivers but do not have enough forms to upload. More forms can be purchased through the in-app purchases. WARNING: Additional forms will not be added to the pending list and cannot be saved."];
...
- (void)SyncICloud {
    ...
    if ([Utilities IsUsingDataSync]) {
        ...
        if (@available(iOS 11.0, *)) {
            ...
            //Use cloud search instead of data sync
            [self CheckSearchCloudPrompt];

            if (![Utilities IsSearchingCloud]) {

                [[NSUserDefaults standardUserDefaults] setObject:@"Yes" forKey:SEARCH_CLOUD_KEY];
            }
...
- (void)CheckSearchCloudPrompt {

    bool firsttime = NO;
    NSString *lastpromptdate = [[NSUserDefaults standardUserDefaults] objectForKey:SYNC_PROMPT_KEY];

    if (lastpromptdate == nil) {

        lastpromptdate = [Utilities NSDateToDateString:[NSDate date]];
        firsttime = YES;
    }

    NSDate *lastdate = [Utilities DateStringToNSDate:lastpromptdate];

    NSUInteger daysbetween = [Utilities DaysBetweenDate:lastdate andDate:[NSDate date]];

    if (daysbetween >= 1 || firsttime) {

        [[NSUserDefaults standardUserDefaults] setObject:[Utilities NSDateToDateString:[NSDate date]] forKey:SYNC_PROMPT_KEY];

        UIAlertController *alert = [self.Alerts CreateOKAlert:^(UIAlertAction *action){} withTitle:@"Attention" withMessage:@"Data Sync'ing with iCloud is no longer necessary. This verion of the app automatically syncs data from your cloud (e.g. Google, Dropbox, Box, or OneDrive) where the pdfs are stored.\n\nYou are using data sync through iCloud which can now be turned off. We suggest disabling iCloud sync in the app settings and allowing it to transfer all your files from icloud onto the ipad overnight. This can be accomplished by disabling data sync in the settings and letting the ipad sit overnight while the waivers are transferred.\n\nEven if you do not disable iCloud sync, data will no longer be stored on iCloud.\n\nPlease contact support if you have any questions or concerns."];
...
#pragma mark - SettingsBackupManagerDelegate
...
- (void)SettingsRestoreComplete:(bool)success withErrorMessage:(NSString *)error {
    ...
    NSString *successStr = @"MRF Settings Transferred";
    NSString *title = @"Success!";

    if(!success) {

        successStr = [NSString stringWithFormat:@"MRF Settings not Transferred: %@", error];
        title = @"Error!";
    }

    UIAlertController *alert = [self.Alerts CreateOKAlert:^(UIAlertAction *action){} withTitle:title withMessage:successStr];
