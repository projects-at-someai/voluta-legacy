...
- (void)viewDidLoad {
    ...
    VideoPreviewView.frame = (self.CurrentDeviceOrientation == UIDeviceOrientationLandscapeLeft ||
                            ...
                            IDCAPTURE_IMGVIEW_LANDSCAPE : IDCAPTURE_IMGVIEW_PORTRAIT;
...
- (UIImage *)ProcessFrontCameraImage:(UIImage *)ImageBuffer {
    ...
    else
    {
        ...
        else
        {
            ...
            float scalar = imageSize.width / 720.0f;

            subRect = CGRectMake(0, 435 * scalar, rotatedImage.size.width, 410 * scalar);
        }
    ...
    NSData *imgData = UIImageJPEGRepresentation(subImage, 1.0);
    NSUInteger numBytes = [imgData length];

    float scalingfactor = 100000.0f / (float)numBytes;

    if (scalingfactor >= 0.75f) {
        scalingfactor = 0.75f;
    }

    imgData = UIImageJPEGRepresentation(subImage,scalingfactor);
...
- (UIImage *)ProcessBackCameraImage:(UIImage *)ImageBuffer {
    ...
    float scalingfactor = 400000.0f / (float)numBytes;
