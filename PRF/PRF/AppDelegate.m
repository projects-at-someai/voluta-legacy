...
+ (void)initialize
{
    ...
    [iVersion sharedInstance].appStoreID = 879280159;
    [iVersion sharedInstance].remoteVersionsPlistURL = [NSString stringWithFormat:@"%@/ver/PRF/versions.plist",VTD_SERVER];
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        */
        ...
        [[QuantcastMeasurement sharedInstance] setupMeasurementSessionWithAPIKey:@"04wgtozvpyerj2fe-9rs0eh1un22jkqqd"
                                                                  userIdentifier:nil labels:nil];
        */

    });

    [defaults setObject:@"Yes" forKey:UPDATE_FLAG];

    /*
    //Check and update PRF v1.x settings
    NSLog(@"Initialize PRF v1.x settings");
    
    NSString *app_setup_key = [defaults objectForKey:[NSString stringWithFormat:@"%@-%@", APP_ID, APP_SETUP_KEY]];
    NSString *v1_setup_key = [defaults objectForKey:@"PRF-setup-complete"];
    
    if ((app_setup_key == nil || ![app_setup_key isEqualToString:@"Yes"]) &&
        v1_setup_key != nil && [v1_setup_key isEqualToString:@"Yes"]) {
        
        NSLog(@"PRF v1.x detected, transferring settings");
        
        DefaultsTransferManager *transfermanager = [[DefaultsTransferManager alloc] initWithDelegate:self];
        
        [transfermanager TransferAll];
...
#pragma mark - DefaultsTransferManagerDelegate
