...
+ (void)initialize
{
    ...
    [iVersion sharedInstance].appStoreID = 769114470;
    [iVersion sharedInstance].remoteVersionsPlistURL = [NSString stringWithFormat:@"%@/ver/TRF/versions.plist",VTD_SERVER];
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if (imagesextracted == nil) {
        
        NSLog(@"Extracting images from core data");
        
        CoreDataManager *_CoreDataManager = [[SharedData SharedInstance] GetCoreDataManager];
        
        NSString *imgsdir = [NSString stringWithFormat:@"%@/Imgs", [[SharedData SharedInstance] GetDocumentsPath]];
        
        if (_CoreDataManager && [_CoreDataManager respondsToSelector:@selector(ExtractAllImages:)]) {
            
            NSLog(@"ExtractAllImages found");
            
            [_CoreDataManager ExtractAllImages:imgsdir];
        }
        else {
            
            NSLog(@"Images not extracted from core data");
        }
