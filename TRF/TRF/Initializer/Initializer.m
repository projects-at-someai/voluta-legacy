...
- (void)InitializeDefaults {
    ...
    if ([defaults objectForKey:SEARCH_CLOUD_KEY] == nil) {

        if (@available(iOS 11.0, *)) {

            [defaults setObject:@"Yes" forKey:SEARCH_CLOUD_KEY];
        }
        else {

            [defaults setObject:@"No" forKey:SEARCH_CLOUD_KEY];
        }
    }
    ...
    if (_VersionHasChanged) {
        ...
        [defaults
         setObject:[Utilities NSDateToDateString:[NSDate date]]
         forKey:UPDATE_DATE_KEY];
    }

    //Track install date
    if ([defaults objectForKey:INSTALL_DATE_KEY] == nil) {

        [defaults setObject:[Utilities NSDateToDateString:[NSDate date]] forKey:INSTALL_DATE_KEY];
    }

    //Track update date
    if ([defaults objectForKey:UPDATE_DATE_KEY] == nil) {

        [defaults setObject:[Utilities NSDateToDateString:[NSDate date]] forKey:UPDATE_DATE_KEY];
    }
