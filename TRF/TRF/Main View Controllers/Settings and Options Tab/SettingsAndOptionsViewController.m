...
- (void)MenuButtonPressed:(UIButton *)button {
    ...
    if ([NewPageName isEqualToString:@"How To"]) {
        ...
        [self.BaseDelegate VCComplete:self destinationViewController:HOWTO_VIEWCONTROLLER];
...
/*
#pragma mark - DBTransferManagerTableCellDelegate
- (void)DBTransferComplete:(DBTransferManagerCell *)cell {
    
    [self DeselectRowAtCell:cell];
}

- (NSString *)GetTransferListFilenameAndPath {
    
    return [[NSBundle mainBundle]pathForResource:DB_TRANSFER_FILE ofType:@"plist"];
}

- (NSString *)GetDBFilenameAndPath {

    NSString *docdir = [[SharedData SharedInstance] GetDocumentsPath];
    
    return [NSString stringWithFormat:@"%@/%@",docdir,DB_NAME];
}

- (NSString *)GetKey {
    
    return @"asd9085rjkvf0j3ceKjfjkqwerpoijusDvnbx0w90ru2enjksdVmv03eiopnsdvavkjn0jwfeiOvdsd";
}
*/
...
#pragma mark - DisposableGripsListEditorTableCellDelegate
- (DisposableGripListManager *)GetDisposableGripListManager {
    
    return [[SharedData SharedInstance] GetDisposableGripListManager];
}

#pragma mark - DisposableTubesListEditorTableCellDelegate
- (DisposableTubeListManager *)GetDisposableTubeListManager {
    
    return [[SharedData SharedInstance] GetDisposableTubeListManager];
}
