...
- (void)InitializeDefaults {
    ...
    if (![self CheckForPList:EMPLOYEELIST_PLST_NAME]) {
        ...
        DLog(@"Employee plist not found, creating a new one in the documents directory");
    ...
    if (_VersionHasChanged || _RegionHasChanged || ![self CheckForPList:FINALIZE_PLIST_NAME]) {
        ...
        DLog(@"Copying %@", FINALIZE_PLIST_NAME);
    ...
    if (![self CheckForPList:HEALTH_PLIST_NAME]) {
        ...
        DLog(@"Copying %@", HEALTH_PLIST_NAME);
    ...
    if (![self CheckForPList:HEALTHITEMSTOCHECK_PLIST_NAME]) {
        ...
        DLog(@"Copying %@", HEALTHITEMSTOCHECK_PLIST_NAME);
    ...
    if (_VersionHasChanged || _RegionHasChanged || ![self CheckForPList:PDFINTER_PLIST_NAME]) {
        ...
        DLog(@"Copying %@", PDFINTER_PLIST_NAME);
    ...
    if (_VersionHasChanged || _RegionHasChanged || ![self CheckForPList:SETTINGSANDOPTIONS_PLIST_NAME]) {
        ...
        DLog(@"Copying %@", SETTINGSANDOPTIONS_PLIST_NAME);
    ...
    if (_VersionHasChanged || _RegionHasChanged || ![self CheckForPList:SETTINGSBACKUP_PLIST_NAME]) {
        ...
        DLog(@"Copying %@", SETTINGSBACKUP_PLIST_NAME);
    ...
    if (_VersionHasChanged || _RegionHasChanged || ![self CheckForPList:SUPPORTINGDOCUMENTS_PLIST_NAME]) {
        ...
        DLog(@"Copying %@", SUPPORTINGDOCUMENTS_PLIST_NAME);
    ...
    if (_VersionHasChanged || _RegionHasChanged || ![self CheckForPList:TABLES_PLIST_NAME]) {
        ...
        DLog(@"Copying %@", TABLES_PLIST_NAME);
    ...
    if (![self CheckForPList:LRF_TRECORD_NAME]) {
        
        DLog(@"Copying %@", LRF_TRECORD_NAME);
        
        [self CopyDefaultPListFromBundle:[DEFAULT_LRF_TRECORD_PLIST_NAME stringByDeletingPathExtension]
                              fromBundle:LRF_TRECORD_BUNDLE_NAME
                        withDestFileName:[_DocumentsDir stringByAppendingPathComponent:LRF_TRECORD_NAME]];
    }
